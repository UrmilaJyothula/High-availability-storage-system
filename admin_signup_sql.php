<?php
 session_start();
 if(!isset($_SESSION["adminsignup_confirm"]))
 {
...
######################################################### CREATING TABLE  SIGNUP FORM #########################################################
   ...
   $sql="CREATE TABLE IF NOT EXISTS Admin_Info(Id INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY,First_name VARCHAR(50),Last_name VARCHAR(50),Email_Id VARCHAR(100),Password VARCHAR(30),reg_date TIMESTAMP)";
...
########################################################## INSERTING DATA INTO TABLE ##########################################################
   ...
   $sql="INSERT INTO Admin_Info (First_name,Last_name,Email_Id,Password) VALUES('$fname','$lname','$eid','$pwd')";
   if(mysqli_query($conn_db,$sql))
   {
    $email_confirm="Admin account has been created successfully";
   }
   else
   {
    #echo "<br>"."Data not inserted successfully"."<br>";
   }
   unset($_SESSION["adminsignup_confirm"]);
